...
function resolveHook(filename, specifier) {
  ...
  if ((0, _compilationCache.belongsToNodeModules)(filename)) return;
...
function js2ts(resolved) {
  ...
  if (!match || _fs.default.existsSync(resolved)) return;
  const tsResolved = match[1] + match[2].replace('js', 'ts');
  if (_fs.default.existsSync(tsResolved)) return tsResolved;
  const tsxResolved = match[1] + match[2].replace('js', 'tsx');
  if (_fs.default.existsSync(tsxResolved)) return tsxResolved;
}
function transformHook(code, filename, moduleUrl) {
  ...
  const {
    cachedCode,
    addToCache
  } = (0, _compilationCache.getFromCompilationCache)(filename, code, moduleUrl);
  ...
  try {
    ...
    const {
      code,
      map
    } = babelTransform(filename, isTypeScript, !!moduleUrl, hasPreprocessor ? scriptPreprocessor : undefined, [require.resolve('./tsxTransform')]);
...
function installTransform() {
  ...
  const revertPirates = _utilsBundle.pirates.addHook((code, filename) => {
    if ((0, _compilationCache.belongsToNodeModules)(filename)) return code;
