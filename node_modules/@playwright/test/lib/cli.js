...
var _profiler = require("./common/profiler");
...
function addTestCommand(program) {
  ...
  command.option('--reporter <reporter>', `Reporter to use, comma-separated, can be ${_configLoader.builtInReporters.map(name => `"${name}"`).join(', ')} (default: "${_configLoader.baseFullConfig.reporter[0]}")`);
  ...
  command.option('--timeout <timeout>', `Specify test timeout threshold in milliseconds, zero for unlimited (default: ${_configLoader.defaultTimeout})`);
...
async function runTests(args, opts) {
  ...
  if (resolvedConfigFile) await configLoader.loadConfigFile(resolvedConfigFile);else await configLoader.loadEmptyConfig(configFileOrDirectory);
  if (opts.deps === false) configLoader.ignoreProjectDependencies();
  const config = configLoader.fullConfig();
  config._internal.cliArgs = args;
  config._internal.cliGrep = opts.grep;
  config._internal.cliGrepInvert = opts.grepInvert;
...
async function listTestFiles(opts) {
  ...
  const runner = new _runner.Runner(configLoader.fullConfig());
...
function resolveReporter(id) {
  if (_configLoader.builtInReporters.includes(id)) return id;
