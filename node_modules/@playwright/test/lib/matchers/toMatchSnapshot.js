...
class SnapshotHelper {
  constructor(testInfo, snapshotPathResolver, anonymousSnapshotExtension, configOptions, nameOrOptions, optOptions) {
    var _mime$getType;
    ...
    testInfo.currentStep.refinedTitle = `${testInfo.currentStep.title}(${_path.default.basename(this.snapshotName)})`;
    options = {
      ...configOptions,
      ...options
    };
    ...
    this.expectedPath = (0, _util.addSuffixToFilePath)(inputFile, '-expected');
  ...
  handleDifferent(actual, expected, previous, diff, diffError, log, title = `${this.kind} comparison failed:`) {
    ...
    if (expected !== undefined) {
      ...
      output.push(`Expected: ${_utilsBundle.colors.yellow(this.expectedPath)}`);
    }
...
function toMatchSnapshot(received, nameOrOptions = {}, optOptions = {}) {
  ...
  if (testInfo.config._internal.ignoreSnapshots) return {
    pass: !this.isNot,
    message: () => ''
  };
  const helper = new SnapshotHelper(testInfo, testInfo.snapshotPath.bind(testInfo), determineFileExtension(received), ((_testInfo$project$_in = testInfo.project._internal.expect) === null || _testInfo$project$_in === void 0 ? void 0 : _testInfo$project$_in.toMatchSnapshot) || {}, nameOrOptions, optOptions);
  if (this.isNot) {
...
async function toHaveScreenshot(pageOrLocator, nameOrOptions = {}, optOptions = {}) {
  var _testInfo$project$_in2, _config$animations, _config$scale, _config$caret;
  const testInfo = (0, _globals.currentTestInfo)();
  ...
  const config = (_testInfo$project$_in2 = testInfo.project._internal.expect) === null || _testInfo$project$_in2 === void 0 ? void 0 : _testInfo$project$_in2.toHaveScreenshot;
  const snapshotPathResolver = testInfo.snapshotPath.bind(testInfo);
  const helper = new SnapshotHelper(testInfo, snapshotPathResolver, 'png', {
  ...
  const customStackTrace = (0, _util.captureStackTrace)(`expect.${this.isNot ? 'not.' : ''}toHaveScreenshot`);
  ...
  if (helper.updateSnapshots === 'none' && !hasSnapshot) return {
    pass: false,
    message: () => `A snapshot doesn't exist at ${helper.snapshotPath}.`
  };
  if (!hasSnapshot) {
    ...
    const {
      ...
      errorMessage,
      log
    } = await page._expectScreenshot(customStackTrace, {
      expected: undefined,
      isNot: false,
      locator,
      comparatorOptions: {
