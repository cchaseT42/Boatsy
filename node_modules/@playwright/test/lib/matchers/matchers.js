...
function toBeInViewport(locator, options) {
  return _toBeTruthy.toBeTruthy.call(this, 'toBeInViewport', locator, 'Locator', async (isNot, timeout, customStackTrace) => {
    return await locator._expect(customStackTrace, 'to.be.in.viewport', {
      isNot,
...
async function toPass(callback, options = {}) {
  ...
  let testStateBeforeToPassMatcher;
  const result = await (0, _utils.pollAgainstTimeout)(async () => {
    try {
      if (testStateBeforeToPassMatcher && testInfo) testInfo._restoreErrorState(testStateBeforeToPassMatcher);
      testStateBeforeToPassMatcher = testInfo === null || testInfo === void 0 ? void 0 : testInfo._saveErrorState();
      await callback();
      if (testInfo && testStateBeforeToPassMatcher && testInfo.errors.length > testStateBeforeToPassMatcher.errors.length) return {
        continuePolling: !this.isNot,
        result: testInfo.errors[testInfo.errors.length - 1]
      };
      return {
        continuePolling: this.isNot,
        result: undefined
