...
function filterForShard(shard, filesByProject) {
  let shardableTotal = 0;
  for (const files of filesByProject.values()) shardableTotal += files.length;
  ...
  for (const [project, files] of filesByProject) {
    ...
    for (const file of files) {
      if (current >= from && current < to) shardFiles.push(file);
      ++current;
    }
    if (shardFiles.length) result.set(project, shardFiles);
  }
  return result;
}