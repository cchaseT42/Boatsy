...
exports.TimeoutWatcher = exports.TaskRunner = void 0;
...
class TaskRunner {
  ...
  async run(context, deadline) {
    const {
      status,
      cleanup
    } = await this.runDeferCleanup(context, deadline);
    const teardownStatus = await cleanup();
    return status === 'passed' ? teardownStatus : status;
  }
  async runDeferCleanup(context, deadline) {
    ...
    const taskLoop = async () => {
      for (const {
        ...
        try {
          ...
          if (teardown) teardownRunner._tasks.unshift({
          ...
          for (const error of errors) {
            ...
            (_this$_reporter$onErr = (_this$_reporter = this._reporter).onError) === null || _this$_reporter$onErr === void 0 ? void 0 : _this$_reporter$onErr.call(_this$_reporter, error);
          }
          if (errors.length) {
    ...
    await Promise.race([taskLoop(), sigintWatcher.promise(), timeoutWatcher.promise]);
    ...
    if (sigintWatcher.hadSignal()) {
      ...
      (_this$_reporter$onErr2 = (_this$_reporter2 = this._reporter).onError) === null || _this$_reporter$onErr2 === void 0 ? void 0 : _this$_reporter$onErr2.call(_this$_reporter2, {
        message: `Timed out waiting ${this._globalTimeoutForError / 1000}s for the ${currentTaskName} to run`
      });
      status = 'timedout';
