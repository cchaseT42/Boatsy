...
function filterStackTrace(e) {
  ...
  Error.prepareStackTrace = (error, structuredStackTrace) => {
    return stackFormatter(error, structuredStackTrace.filter(callSite => {
      ...
      return !fileName.startsWith(PLAYWRIGHT_TEST_PATH) && !fileName.startsWith(PLAYWRIGHT_CORE_PATH) && !(0, _utils.isInternalFileName)(fileName, functionName);
    }));
  };
...
function captureStackTrace(customApiName) {
  const stackTrace = (0, _utils.captureStackTrace)();
  const frames = [];
  const frameTexts = [];
