...
(0, _utils.addStackIgnoreFilter)(frame => frame.file.startsWith(path.dirname(require.resolve('../package.json'))));
...
const playwrightFixtures = {
  ...
  connectOptions: [({}, use) => {
    ...
    if (!wsEndpoint) return use(undefined);
  ...
  _setupContextOptionsAndArtifacts: [async ({
    ...
    const stopTraceChunkOnContextClosure = async tracing => {
    ...
    for (const browserType of [playwright.chromium, playwright.firefox, playwright.webkit]) {
      ...
      if (_reuseContext) existingContexts.forEach(c => reusedContexts.add(c));else await Promise.all(existingContexts.map(onDidCreateBrowserContext));
    }
    {
    ...
    const stopTraceChunkOnTestFinish = async tracing => {
